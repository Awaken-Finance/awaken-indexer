...
namespace SwapIndexer.Tests.Provider;
...
public class MockAElfDataProvider : IAElfDataProvider
{
    ...
    public async Task<long> GetDecimaleAsync(string contractAddress, string chainId, string symbol)
    {
        if ("USDT" == symbol)
        {
            return 6;
    ...
    public async Task<long> GetTransactionFeeAsync(string contractAddress, string chainId, string txnId)
    {
        switch (txnId)
        {
