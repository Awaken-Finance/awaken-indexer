...
public abstract class LimitOrderProcessorBase<TEvent> : LogEventProcessorBase<TEvent> where TEvent : IEvent<TEvent>, new()
{
    ...
    public async Task<long> GetTransactionFeeAsync(Transaction transaction)
    {
        try
        {
            ...
            {
                Logger.LogDebug($"txn: {transaction.TransactionId}, can't find NonIndexed in extraProperties: {transaction.LogEvents?.FirstOrDefault(l => l.EventName == nameof(TransactionFeeCharged))
                    ?.ExtraProperties}");
                return 0;
            }
            var transactionFeeCharged = TransactionFeeCharged.Parser.ParseFrom(
                ByteString.FromBase64(extraProperties["NonIndexed"]));
            Logger.LogDebug($"txn: {transaction.TransactionId}, fee amount: {transactionFeeCharged.Amount}, NonIndexed: {extraProperties["NonIndexed"]}");
